...
def vgg16(weights_path : Optional[str] = None):
    model = VGG(make_features(channels_cfgs), weights_path = weights_path)
    return model

class VGG(nn.Module):
    def __init__(self, features, class_num : int = 100, init_weights : bool = False, weights_path : Optional[str] = None):
        super().__init__()
...
def make_features(ch_cfgs : List):
    layers = []
    in_channels = 3
