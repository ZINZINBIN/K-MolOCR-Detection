...
class AuxiliaryConvolutions(nn.Module):
    ...
    def forward(self, conv7_feats:torch.Tensor):
        """
        Forward propagation.
...
class SSD300(nn.Module):
    ...
    def predict(self, predicted_locs:torch.Tensor, predicted_scores:torch.Tensor, min_score:torch.Tensor, max_overlap, top_k):
        ...
        for i in range(batch_size):
            ...
            decoded_locs = cxcy_to_xy(gcxgcy_to_cxcy(predicted_locs[i], self.priors_cxcy.to(device)))  # (8732, 4), these are fractional pt. coordinates
