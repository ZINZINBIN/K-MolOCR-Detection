...
def get_custom_transforms(*args):
    train_transform = A.Compose(
        [   
            A.ColorJitter(contrast=0.5),
            A.HorizontalFlip(p = 0.5),
            A.Rotate(p=0.5),
            A.Resize((args['resize_height','resize_width'])),
            ToTensorV2(),
...
def calculate_mAP(det_boxes, det_labels, det_scores, true_boxes, true_labels, max_overlap_thres, device):
    ...
    for c in range(1, n_classes):
        ...
        n_class_true = (true_labels == c).sum().item()
        ...
        for d in range(n_class_detections):
            ...
            if max_overlap.item() > max_overlap_thres:
        ...
        cumul_recall = cumul_true_positives / n_class_true  # (n_class_detections)
